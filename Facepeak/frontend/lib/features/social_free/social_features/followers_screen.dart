...
class _FollowersScreenState extends State<FollowersScreen>
  ...
  Future<void> _load({bool refresh = false}) async {
    ...
    try {
      final page = await SocialFollowService.getFollowersPage(
      ...
      debugPrint("❌❌❌ FOLLOWERS_SCREEN load ERROR = $e");
      debugPrint("❌❌❌ FOLLOWERS_SCREEN load STACK = $s");

      if (!mounted) return;
      setState(() => _loading = false);
      _toast("Could not load followers");
    }
  }
  ...
  Future<void> _loadMore() async {
    ...
    try {
      ...
      debugPrint("❌❌❌ FOLLOWERS_SCREEN loadMore ERROR = $e");
      debugPrint("❌❌❌ FOLLOWERS_SCREEN loadMore STACK = $s");
  ...
  List<Map<String, dynamic>> _items(Map<String, dynamic> page) {
    final raw = page["items"] ?? page["followers"];
  ...
  Future<void> _followBack(Map<String, dynamic> u) async {
    final id = _userId(u);
    if (id <= 0 || _actionLoading || _isFollowing(u)) return;
    ...
    setState(() {
      _actionLoading = true;
      _users = _users.map((x) {
        if (_userId(x) != id) return x;
        return {...x, "is_following": true};
      }).toList();
    });

    try {
      await SocialFollowService.followUser(targetUserId: id);

      if (!mounted) return;
      _toast("Followed back");
    } catch (e) {
      debugPrint("❌❌❌ FOLLOWERS_SCREEN followBack ERROR = $e");
      ...
      setState(() {
        _users = _users.map((x) {
          if (_userId(x) != id) return x;
          return {...x, "is_following": false};
        }).toList();
      });

      _toast("Could not follow back");
    } finally {
      if (mounted) setState(() => _actionLoading = false);
    }
  }

  Future<void> _removeFollower(Map<String, dynamic> u) async {
    final id = _userId(u);
    ...
    try {
      await SocialFollowService.removeFollower(followerId: id);

      if (!mounted) return;
      _toast("Follower removed");
    } catch (e) {
      debugPrint("❌❌❌ FOLLOWERS_SCREEN remove ERROR = $e");
      ...
      _toast("Could not remove follower");
    } finally {
      if (mounted) setState(() => _actionLoading = false);
    }
  }
  ...
  Future<void> _blockUser(Map<String, dynamic> u) async {
    ...
    try {
      ...
      debugPrint("❌❌❌ FOLLOWERS_SCREEN block ERROR = $e");
                    ...
                    children: [
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                ...
                                Text(
                                  following
                                      ? "Follows you • mutual connection"
                                      : "Follower • not followed back yet",
                                  maxLines: 1,
                                  overflow: TextOverflow.ellipsis,
                                  style: TextStyle(
                      ...
                      if (!following) ...[
                        _sheetTile(
                          ...
                          title: "Follow back",
                          subtitle: "Start a mutual connection",
                          color: gold2,
                          onTap: () {
                            Navigator.pop(context);
                            _followBack(u);
                      ...
                      _sheetTile(
                        ...
                        subtitle: "They will no longer follow you",
                        color: gold2,
                        onTap: () {
                          Navigator.pop(context);
                          _removeFollower(u);
                      ...
                      _sheetTile(
                        ...
                        title: "Block user",
                        subtitle: "Remove connection and stop interactions",
                        color: danger,
                        onTap: () {
                          Navigator.pop(context);
                          _blockUser(u);
